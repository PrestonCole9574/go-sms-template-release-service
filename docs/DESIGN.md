...
> Design notes for **go-sms-template-release-service** — a runnable go example that Small Go service for approving and releasing SMS templates with Infrai.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.sms.signature.create(...)`).
- **Feature code** — the actual task: sms-template-management.
- **Configuration** — the API key is read from the `INFRAI_API_KEY` environment variable; no secret is ever hard-coded.

## Capabilities used

- `sms.signature.create` — mapped to `POST /v1/sms/signature/create`.
- `sms.template.create` — mapped to `POST /v1/sms/template/create`.
